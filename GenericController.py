...
from joystickstuff import Button, TriggerAxis, Hat
...
class LoadGenericController():
    ...
    def update(self):
        for item in self.buttondict:
            check = self.buttondict[item].update_self()
            if check:
                self.actioncount = self.actioncount + 1
        for item in self.axisdict:
            check = self.axisdict[item].update_self()
            if check:
                self.actioncount = self.actioncount + 1
        for item in self.sticklist:
            check = item.update_self()
            if check:
                self.actioncount = self.actioncount + 1
        for item in self.hatdict:
            # check = item.UpdateSelf(item)
            check = self.hatdict[item].update_self()
            if check:
                self.actioncount = self.actioncount + 1
