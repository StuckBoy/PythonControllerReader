...
class PlayStation5Controller():
    buttonlist = [xbutton, obutton, sqbutton, tbutton, upbutton, downbutton, leftbutton, rightbutton, lbump, rbump,
                  mbump, sharebutton, pausebutton, micbutton, psbutton]
    ...
    def update(self):
        for item in self.buttonlist:
            check = item.update_self(self.ID)
            if check:
